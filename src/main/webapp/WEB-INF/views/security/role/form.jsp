...
<div class="content">
<div class="table">
<div class="contentNav"><h1>用户角色管理</h1></div>
...
<tbody>
<tr>
<td width="240" align="right" nowrap="nowrap"><span class="red" style="color: red;" >*</span>显示名称：</td>
<td colspan="2">
	<form:input path="showName" cssClass="input5 fontMar" />
</td>
</tr>
<tr>
<td width="240" align="right" nowrap="nowrap"><span class="red" style="color: red;" >*</span>名称：</td>
<td colspan="2">
...
<c:forEach var="auth" items="${authorities }">
	<c:choose>
		...
		<input type="checkbox" id="auth${auth.id }" name="authoritiesTrans[${auth.id }]" value="${auth.id}" checked="checked"/>
		<label for="auth${auth.id }">${auth.name }</label>
		</c:when>
		<c:otherwise>
		<input type="checkbox" id="auth${auth.id }" name="authoritiesTrans[${auth.id }]" value="${auth.id}" />
		<label for="auth${auth.id }">${auth.name }</label>
		</c:otherwise>
	</c:choose>
</c:forEach>
