...
<script type="text/javascript">
	$(function(){
		...
		if(currentUrl && currentUrl.indexOf("edit") !== -1){
			...
			$("#password-tip").empty().append('<span class="red" style="color: red;" >（如不修改，请留空）</span>密码：');
		}
	});
</script>
</head>
...
<div class="content">
<div class="table">
<div class="contentNav"><h1>用户管理</h1></div>
...
<tbody>
<tr>
<td width="240" align="right" nowrap="nowrap"><span class="red" style="color: red;" >*</span>用户名：</td>
<td colspan="2">
	<form:input path="username" id="username" cssClass="input5 fontMar" />
</td>
</tr>
<tr>
<td width="240" align="right" nowrap="nowrap" id="password-tip"><span class="red" style="color: red;" >*</span>密码：</td>
<td colspan="2">
...
<c:forEach var="role" items="${roles }">
	<c:choose>
		...
		<input type="checkbox" id="role${role.id }" name="rolesTrans[${role.id }]" value="${role.id}"  checked="checked"/>
		<label for="role${role.id }">${role.name }</label>
		</c:when>
		<c:otherwise>
		<input type="checkbox" id="role${role.id }" name="rolesTrans[${role.id }]" value="${role.id}"/>
		<label for="role${role.id }">${role.name }</label>
		</c:otherwise>
	</c:choose>
</c:forEach>
